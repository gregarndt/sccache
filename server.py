...
def _run_command(job):
    ...
    '''
    sys.stderr.write("dia: Running job with compiler"
    id = job['id']
    ...
    if preprocessed:
        ...
        sys.stderr.write("dia: Preprocessed. Cache key: {}".format(cache_key)

        if not 'SCCACHE_RECACHE' in os.environ and storage:
            # Get cached data if there is.
            sys.stderr.write("dia: Attempting to get cached data for {}".format(cache_key)
            ...
            if data:
                try:
                    ...
                    pass
            if cache:
                sys.stderr.write("dia: Cache created"
    ...
    if not ret and can_cache and storage and \
        ...
        try:
            cache = CacheData()
            sys.stderr.write("dia: Cache Data instance created"
            cache['stdout'] = stdout
            cache['stderr'] = stderr
            for key, path in outputs.items():
                with open(path, 'rb') as f:
                    sys.stderr.write("dia: Reading data for {}".format(path)
                    cache[key] = f.read()
        except:
            sys.stderr.write("dia: Caught error when creating Cache Data instance"
            cache = None
    ...
    if cache:
        for retry in range(0, 3):
            if storage.put(cache_key, cache.data):
                sys.stderr.write("dia: Attempting to store data for {}".format(cache_key)
                yield dict(status='stored', stats=storage.last_stats)
                break
            else:
                sys.stderr.write("dia: Couldn't store data for {}".format(cache_key)
